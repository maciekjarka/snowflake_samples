CREATE OR REPLACE PROCEDURE COPILOT_DB.STATS.CALL_CORTEX_AGENT_PROC("QUERY" VARCHAR, "LIMIT" NUMBER(38,0))
RETURNS VARCHAR
LANGUAGE PYTHON
RUNTIME_VERSION = '3.9'
...
def call_cortex_agent_proc(query: str, limit: int = 10):
    ...
    # Change semantic model location here:
    SEMANTIC_MODELS = "@COPILOT_DB.STATS.COPILOT_STG/products.yaml"
    ...
    try:
        ...
        #data = [str(resp)]
        #df = session.create_dataframe(data, schema=["response"])
        #df.write.mode("overwrite").save_as_table("COPILOT_DB.STATS.T_RESPONSE")
